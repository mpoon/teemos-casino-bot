...
public class SpectateAnyone
{
	...
	public static void setupClient(String configPath)
	{
		// Read in the config
		...
		if (conf.exists())
		{
			...
			catch (IOException e)
			{
				JOptionPane.showMessageDialog(
						f,
						"Encountered an error when parsing spectateConfig",
					    "Error",
		...
		// Set up spectateConfig if needed
		if (newinfo)
		{
			try
			{
			...
			{
				System.out.println("Encountered an error when creating spectateConfig:");
				e.printStackTrace();
			}
		}
		...
		if (lol == null || air == null)
		{
			JOptionPane.showMessageDialog(
					f,
					"The installation location does not appear to be valid, make sure lollocation is correct in spectateConfig.",
				    "Error",
				    JOptionPane.ERROR_MESSAGE);
			System.exit(0);
		}
		...
		// Connect
        if(Boolean.parseBoolean(params.get("dev"))) {
            client = new LoLRTMPSClient(params.get("region"), params.get("version"), params.get("dev_user"), params.get("dev_pass"));
        }
        else {
	...
	public static synchronized Process handleSpectate()
	{
		...
		try
		{
			...
			else
			{
				...
				// Otherwise, just spectate
					...
					return doSpectate(ip, port, key, gameID);
			}
		}
		catch (IOException e)
		{
	...
	public static Process doSpectate(String ip, int port, String key, int gameID)
	{
